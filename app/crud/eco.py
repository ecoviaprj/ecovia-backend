...
from app.utils.verify_image import verify_image_with_gradio
...
def create_eco_task(db: Session, task: schemas.EcoTaskCreate):
    ...
    if not action:
        raise HTTPException(status_code=404, detail="Eco task/action does not exist.")

    existing = db.query(models.EcoTask).filter_by(
        user_id=task.user_id, task_id=task.task_id
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail="You already submitted this task.")

    task_data = task.dict()
    is_verified = verify_image_with_gradio(task.image_path)
    task_data["status"] = "verified" if is_verified else "rejected"
